    ...
    public class IsNullComparisonRefactorRule : ISqlRefactorRule
    {
        ...
        public RuleResult Apply(TSqlFragment fragment, RefactorContext context)
        {
            ...
            if (fragment is BooleanExpression boolExpr)
            {
                ...
                {
                    var desc = visitor.ChangeDetail ?? "Optimized ISNULL comparison";
        ...
        private class IsNullRewriteVisitor : SqlXmlAnalyzer.Core.Refactoring.BooleanExpressionReplacementVisitor
        {
            public string? FoldedColumnName { get; private set; }
            ...
            private bool TryOptimize(ScalarExpression left, ScalarExpression right, BooleanComparisonType compType, out BooleanExpression optimized)
            {
                ...
                if (TryGetIsNullPattern(left, out var colRef, out var defaultValue))
                {
                    if (colRef != null && IsLiteralOrConstant(defaultValue) && IsLiteralOrConstant(right))
                    {
                        ...
                        {
                            FoldedColumnName = GetColumnName(colRef);
                            var satisfaction = satisfies ? "satisfied" : "not satisfied";
                            ChangeDetail = $"Optimized ISNULL comparison on column {FoldedColumnName} ({satisfaction} by default value)";

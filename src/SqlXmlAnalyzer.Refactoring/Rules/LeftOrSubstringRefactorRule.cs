    ...
    public class LeftOrSubstringRefactorRule : ISqlRefactorRule
    {
        ...
        public RuleResult Apply(TSqlFragment fragment, RefactorContext context)
        {
            ...
            if (fragment is BooleanExpression boolExpr)
            {
                ...
                {
                    var desc = visitor.ChangeDetail ?? "Optimized LEFT/SUBSTRING comparison";
                ...
                if (TryGetLeftOrSubstringPattern(left, out var colRef, out int length))
                {
                    if (right is StringLiteral strLit && strLit.Value.Length == length)
                    {
                        ...
                        if (!ContainsLikeWildcards(strLit.Value))
                        {
                            ...
                            _context.Log($"Optimized LEFT/SUBSTRING comparison on column {colName} to LIKE");
                            ChangeDetail = $"Optimized LEFT/SUBSTRING comparison on column {colName} to LIKE";
                            return true;
                        }
                    }
                }

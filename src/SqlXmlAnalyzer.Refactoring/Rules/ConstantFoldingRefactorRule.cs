    ...
    public class ConstantFoldingRefactorRule : ISqlRefactorRule
    {
        ...
        public RuleResult Apply(TSqlFragment fragment, RefactorContext context)
        {
            ...
            if (fragment is BooleanExpression boolExpr)
            {
                ...
                {
                    var colName = visitor.FoldedColumnName ?? "column";
                    return new RuleResult(replaced, true, $"Folded constant arithmetic on column {colName}");
                }
            }
            else
            {
                ...
                {
                    var colName = visitor.FoldedColumnName ?? "column";
                    return new RuleResult(fragment, true, $"Folded constant arithmetic on column {colName}");
                }
            }
        ...
        private class RewriteVisitor : SqlXmlAnalyzer.Core.Refactoring.BooleanExpressionReplacementVisitor
        {
            ...
            protected override BooleanExpression ReplaceExpression(BooleanExpression expression)
            {
                if (expression is BooleanComparisonExpression compExpr)
                {
                    ...
                    {
                        FoldedColumnName = GetColumnName(colRefRev!);
                        return new BooleanComparisonExpression
                        {
                            ComparisonType = compExpr.ComparisonType,

...
namespace SqlXmlAnalyzer.Core.Rules
{
    public class UdfAndTableVariableRule : IPlanAnalyzerRule
    {
        ...
        public AnalysisResult? Analyze(XElement relOp, XNamespace ns)
        {
            ...
            double totalActualRows = (double)(facts.RowsPerExecution.Value ?? 0);
            string msg = "";
            ...
            if (totalActualRows > 0 && estimatedRows > 0)
            {
                ...
                {
                    severity = "Critical";
                    msg = $"表变量或 TVF 导致了严重的基数估算失败！优化器预估 {estimatedRows:N0} 行，每次执行实际输出 {totalActualRows:N0} 行（偏差 {ratio:F0} 倍）。\n这会彻底带坏后续 JOIN 算子的选择，强烈建议改用 #临时表 (Temp Table) 以获取直方图统计信息支持。";
                }
                else
                {
                    msg = $"检测到使用了表变量、TVF 或 UDF。\n预估行数: {estimatedRows:N0}，每次执行实际行数: {totalActualRows:N0}。\n注意：标量 UDF 会导致逐行执行 (Row-by-Row) 且禁用并行计划；表变量缺乏统计信息。";
                }
            }
            else
            {

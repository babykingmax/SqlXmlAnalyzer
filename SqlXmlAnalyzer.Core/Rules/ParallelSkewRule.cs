...
namespace SqlXmlAnalyzer.Core.Rules
{
    public class ParallelSkewRule : IPlanAnalyzerRule
    {
        ...
        public AnalysisResult? Analyze(XElement relOp, XNamespace ns)
        {
            ...
            if (facts.Execution.Parallel != true || facts.WorkerRows is not { Count: > 1 } workers) return null;
            double totalRows = (double)workers.Total;
            int actualDop = workers.Count;
            double maxWorkerRows = (double)workers.Maximum;

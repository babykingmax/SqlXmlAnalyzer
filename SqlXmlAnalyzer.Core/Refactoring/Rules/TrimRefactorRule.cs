                ...
                if (TryStripTrims(left, out var colRef, out bool hasLTrim, out bool hasRTrim))
                {
                    if (right is StringLiteral strLit)
                    {
                            ...
                            !char.IsWhiteSpace(strLit.Value[strLit.Value.Length - 1]))
                        {
                            ...
                            if (hasLTrim)
                            {
                                _context.Warn($"Removed LTRIM on column {GetColumnName(colRef!)}. Note: This assumes column values do not contain leading spaces.");
                            }
                            _context.Log($"Optimized LTRIM/RTRIM on column {GetColumnName(colRef!)}");
                            return true;
                        }
                    }
                }

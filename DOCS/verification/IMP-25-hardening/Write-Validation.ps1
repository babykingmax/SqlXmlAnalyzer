...
foreach ($configuration in @('Debug', 'Release')) {
    ...
    if (-not $wpf.Passed -or $wpf.BindingErrors -ne 0 -or $wpf.Matrix.Count -ne 54 -or
        ...
        $wpf.Checks -notcontains 'Both expanded sidebars cycle 2 keeps every column inside the scroll extent') { throw "$configuration WPF verification failed" }
...
[ordered]@{
    task = 'IMP-25 review fixes: restored sidebar extent, eligible focus cycling and operator details'

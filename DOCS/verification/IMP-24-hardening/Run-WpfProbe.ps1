...
$probeDirectory = Join-Path ([IO.Path]::GetTempPath()) ('SqlXmlAnalyzer-IMP24-WPF-' + [Guid]::NewGuid().ToString('N'))
...
dotnet build (Join-Path $probeDirectory 'Probe.csproj') -c $Configuration -m:1 "-p:IntermediateOutputPath=obj/imp24/$Configuration/" -v minimal

...
try {
    $results = foreach ($mode in @('Debug', 'Release')) {
        $stdout = Join-Path $repository ".tmp.imp16-review-cli-$($mode.ToLowerInvariant()).json"
        $stderr = Join-Path $repository ".tmp.imp16-review-cli-$($mode.ToLowerInvariant()).stderr"
        ...
        foreach ($candidate in $candidates) {
            if ($candidate.RuleVersion -ne '2.1.1') { throw 'Wrong rule version' }
            if (@($candidate.Evidence | Where-Object { $_.Name -eq 'ScoreModel' -and $_.Value -eq 'index-evidence-score/2.0.1' }).Count -ne 1) { throw 'Missing model' }
        ...
        $breakdown = @($candidates[0].Evidence | Where-Object Name -eq 'ScoreBreakdown')[0].Value
        if ($breakdown -notmatch '85/100') { throw "Parenthesized predicates should score 85/100: $breakdown" }

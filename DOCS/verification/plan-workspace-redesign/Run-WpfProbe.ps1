param([ValidateSet('Debug','Release')][string]$Configuration = 'Debug', [switch]$TooltipsOnly)
$ErrorActionPreference = 'Stop'
$repository = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../../..'))
$probeDirectory = Join-Path ([IO.Path]::GetTempPath()) ('SqlXmlAnalyzer-PlanUi-' + [Guid]::NewGuid().ToString('N'))
$outputDirectory = Join-Path $PSScriptRoot ('runs/' + (Get-Date -Format 'yyyyMMddTHHmmss') + '-' + $Configuration)
New-Item -ItemType Directory -Path $probeDirectory,$outputDirectory -Force | Out-Null
...
$probeArguments = @(('"{0}"' -f $repository), ('"{0}"' -f $outputDirectory))
if ($TooltipsOnly) { $probeArguments += '--tooltips' }
$probe = Start-Process -FilePath (Join-Path $probeDirectory "bin/$Configuration/net8.0-windows/Probe.exe") -ArgumentList $probeArguments -WindowStyle Hidden -PassThru -Wait -RedirectStandardOutput (Join-Path $outputDirectory 'stdout.log') -RedirectStandardError (Join-Path $outputDirectory 'stderr.log')

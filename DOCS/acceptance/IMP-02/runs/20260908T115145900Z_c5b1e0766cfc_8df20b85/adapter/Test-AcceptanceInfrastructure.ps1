...
function New-RedactionObservation {
    [pscustomobject]@{ status='Ready'; CanExport=$true; literalRetained=$false; serverRetained=$false;
        aliasRetained=$false; tableMasked=$true; noSentinel=$true; sourceUnchanged=$true; payloadWellFormed=$true;
        unsupportedBlocked=$true; unsupportedHasNoPayload=$true; unsupportedSummarySafe=$true }
}
Check 'R01 requires a ready payload before accepting missing sentinels' {
    foreach ($state in @('Failed','Blocked','Cancelled')) {
        $o=New-RedactionObservation; $o.status=$state; $o.CanExport=$false; $o.payloadWellFormed=$false
        Assert-True ((Get-RedactionProbeCheck $o).status -eq 'ProbeExecutionFailed') 'Empty failure payload was accepted.'
    }
}
Check 'R01 accepts the supported and fail-closed fixture contract' {
    Assert-True ((Get-RedactionProbeCheck (New-RedactionObservation)).status -eq 'ProbeConditionMet') 'Valid redaction contract was rejected.'
}
Check 'R01 rejects every failed preservation or privacy condition' {
    foreach ($property in @('tableMasked','noSentinel','sourceUnchanged','unsupportedBlocked','unsupportedHasNoPayload','unsupportedSummarySafe')) {
        $o=New-RedactionObservation; $o.$property=$false
        Assert-True ((Get-RedactionProbeCheck $o).status -eq 'NotMet') "Privacy failure $property was accepted."
    }
}
Check 'R01 rejects malformed or string-valued boolean observations' {
    $o=New-RedactionObservation; $o.CanExport='false'
    Assert-Throws { Get-RedactionProbeCheck $o }
    $o=New-RedactionObservation; $o.PSObject.Properties.Remove('status')
    Assert-Throws { Get-RedactionProbeCheck $o }
}

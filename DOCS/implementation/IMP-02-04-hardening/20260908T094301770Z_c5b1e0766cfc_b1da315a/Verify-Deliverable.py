...
import hashlib,json,re,subprocess,struct,datetime
...
for name in owned+docs+adapters:
    target=run/'source-snapshots'/(name+'.snapshot')
...
links=[]
future={str((run/n).resolve()) for n in ['deliverable-validation.json','artifact-manifest.json']}
for doc in docs:
    p=repo/doc

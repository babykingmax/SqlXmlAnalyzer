...
$probe = Join-Path $repository 'bin/imp10-ui-probe'
[void][IO.Directory]::CreateDirectory($probe)
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'IdentityUiProbe.cs.txt') -Destination (Join-Path $probe 'Program.cs') -Force
$project = (Get-Content -LiteralPath (Join-Path $PSScriptRoot 'IdentityUiProbe.csproj.txt') -Raw).Replace('APP_OUTPUT', (Join-Path $repository 'bin/Release/net8.0-windows'))
[IO.File]::WriteAllText((Join-Path $probe 'IdentityUiProbe.csproj'), $project)
dotnet build (Join-Path $probe 'IdentityUiProbe.csproj') -c Release *> (Join-Path $PSScriptRoot 'ui-build.log')
...
$start.ArgumentList.Add((Join-Path $probe 'bin/Release/net8.0-windows/IdentityUiProbe.dll'))

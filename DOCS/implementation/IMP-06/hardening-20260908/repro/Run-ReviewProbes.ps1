param(
    [string]$ArtifactsRoot = 'E:\SqlXmlAnalyzer\bin\imp06-hardening-20260908\bin'
)
...
$encodingObserved = foreach ($fixture in @('invalid-utf8-byte','valid-utf16-no-bom','valid-utf32-no-bom')) {
    ...
    [pscustomobject]@{File="$fixture.sqlplan";StrictXmlError=$strictError;SharedInputStatus=$recognized.Status.ToString();GuiOpenStatus=$opened.Status.ToString();HasDocument=($null -ne $recognized.Document);HasReplacementCharacter=($null -ne $recognized.Document -and $recognized.Document.ToString().Contains([char]65533))}
}
...
$failNames = @('correct-relop-namespace','wrong-relop-namespace','invalid-utf8-byte')
foreach ($result in $results) {
    $expected = if ($result.Name -in $failNames) { 1 } else { 0 }
    if ($result.ExitCode -ne $expected) { throw "Unexpected CLI result: $($result.Name)" }
}
'All nine CLI subprocess outcomes match the hardened contract.'

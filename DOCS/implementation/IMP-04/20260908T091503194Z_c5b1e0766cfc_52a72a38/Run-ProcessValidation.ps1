...
function Invoke-Dotnet([string[]]$Arguments, [string]$Stem) {
    ...
    try {
        ...
        if (-not $process.WaitForExit(60000)) { $process.Kill($true); throw 'Probe process timed out.' }
        [IO.File]::WriteAllText("$Stem.stdout.txt", $stdout.Result)
        [IO.File]::WriteAllText("$Stem.stderr.txt", $stderr.Result)
        $record = [ordered]@{ executable = 'dotnet'; arguments = $Arguments; processId = $childId; startUtc = $start.ToString('o'); exitCode = $process.ExitCode }
        $commands.Add($record)
        return $record
    }
...
try {
    ...
    foreach ($configuration in @('Debug', 'Release')) {
        ...
        $unknownPath = Join-Path $configurationPath 'unknown-error'
        $unknown = Invoke-Dotnet @((Join-Path $temporaryProject "bin/$configuration/net8.0/DiagnosticProbe.dll"), $unknownPath) (Join-Path $configurationPath 'unknown-error')
        ...
        $results += [ordered]@{ configuration = $configuration; scenario = 'unknown-error'; passed = $unknown.exitCode -eq 1 -and $verification.Passed -and $verification.Configuration -eq $configuration -and $policyPassed; logPolicyPassed = $policyPassed; verification = $verification }
